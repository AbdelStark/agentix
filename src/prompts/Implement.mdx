...
- No fake implementations, placeholders, TODO-driven exits, or silent fallbacks.
- No speculative abstractions without an active scenario proving need.
- No merge-ready output unless tests, invariants, and scenario coverage are real and passing.
...
### Step 0.5: Lock scenario coverage before coding

- Build a scenario checklist from `gherkinScenarios`.
- Keep scenario IDs visible in tests (test names, comments, or table-driven cases).
- Do not close the unit with uncovered scenarios.
- Each domain invariant must be asserted by at least one test.
...
- Repeat until every Gherkin scenario has at least one test proving its Then clauses.
